...
function RoleBadge({ role }) {
  const isOwner = role === 'SchoolOwner'
  return (
    <span
      style={{
...
export default function UsuariosEscola() {
  ...
  const fetchUsers = useCallback(async () => {
    ...
    setError(null)
    try {
      const res = await listSchoolUsers(schoolId, {
  ...
  }, [schoolId, debouncedSearch, roleFilter, statusFilter, page, pageSize])
  ...
  const handleRemoverAcesso = useCallback(async () => {
    if (!removeModal || !schoolId) return
    setRemoving(true)
    try {
      await deleteSchoolUser(schoolId, removeModal.user_id)
      setRemoveModal(null)
      setToast('Usuário removido da escola com sucesso.')
      fetchUsers()
    } catch (err) {
      if (err.status === 403) setPermissionDenied(true)
  ...
  }, [schoolId, removeModal, fetchUsers])
      ...
      <div style={styles.cardGrande}>
        ...
        <div style={styles.cabecalho}>
          <div style={styles.cabecalhoEsq}>
            {loadingSchool ? (
              ...
              <>
                <h1 style={styles.titulo}>Usuários da Escola</h1>
                <p style={styles.subtitulo}>{schoolName}</p>
              </>
            )}
          </div>
          {!loadingSchool && (
            <div style={styles.cabecalhoBotoes}>
              <Link to={`/admin/escolas/${schoolId}/usuarios/novo`} style={styles.btnPrimario} className="btn-hover">
                Novo usuário
              </Link>
        ...
        <div style={styles.filtros}>
          ...
          >
            ...
            <option value="SchoolOwner">SchoolOwner</option>
            <option value="SchoolStaff">SchoolStaff</option>
              ...
              <tbody>
                  ...
                  <tr>
                    <td colSpan={6} style={styles.tdVazio}>
                      <div style={styles.emptyWrap}>
                        <p style={styles.emptyTitulo}>Nenhum usuário vinculado a esta escola.</p>
                        <Link to={`/admin/escolas/${schoolId}/usuarios/novo`} style={styles.btnPrimario} className="btn-hover">
                          Novo usuário
                        </Link>
                      </div>
                ...
                {!loading && data?.data?.length > 0 && data.data.map((user) => (
                  <tr key={user.user_id}>
                    ...
                    <td style={styles.td}>{getSchoolScopeSummary(user, schoolId)}</td>
                      ...
                      <div style={styles.acoesCel} data-menu-wrap>
                        ...
                        {menuAberto === user.user_id && (
                          <div style={styles.dropdown}>
                            <Link
                              to={`/admin/escolas/${schoolId}/usuarios/${user.user_id}/editar`}
        ...
        {removeModal && (
          <div style={styles.modalOverlay} role="dialog" aria-modal="true" aria-labelledby="modal-remove-school-user-title">
            <div style={styles.modal}>
              <h2 id="modal-remove-school-user-title" style={styles.modalTitulo}>Remover acesso deste usuário à escola?</h2>
              <p style={styles.modalTexto}>
                Ele perderá acesso às informações desta escola.
              </p>
