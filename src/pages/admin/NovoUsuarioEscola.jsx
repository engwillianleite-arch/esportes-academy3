...
export default function NovoUsuarioEscola() {
  const { id: schoolId } = useParams()
  const navigate = useNavigate()
  const location = useLocation()
  const listUrl = `/admin/escolas/${schoolId}/usuarios`
  const returnToEscola = location.state?.returnTo || `/admin/escolas/${schoolId}`
  ...
  const handleSubmit = async (e) => {
    ...
    setSubmitError(null)
    try {
      await createSchoolUser(schoolId, {
        name: form.name.trim(),
    ...
    <AdminLayout breadcrumb={breadcrumb}>
      <div style={styles.cardGrande}>
        <div style={styles.cabecalho}>
          <h1 style={styles.tituloPagina}>Novo usuário da escola</h1>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div style={styles.secao}>
            ...
            <p style={{ margin: 0, fontSize: 14, color: 'var(--grafite-tecnico)', opacity: 0.8 }}>
              O usuário terá acesso apenas a esta escola ({schoolName}).
            </p>
          </div>
