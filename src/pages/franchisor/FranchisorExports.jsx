...
const TYPE_OPTIONS = [
  { value: 'todos', label: 'Todos' },
  { value: 'CONSOLIDATED_REPORT', label: 'Relatório consolidado' },
  { value: 'SCHOOL_REPORT', label: 'Relatório por escola' },
  { value: 'CAMPAIGN_RESULTS', label: 'Resultados de campanha' },
]
...
export default function FranchisorExports() {
  ...
  const formatFromUrl = searchParams.get('format') || 'todos'
  ...
  const [format, setFormat] = useState(formatFromUrl)
  ...
  }, [debouncedSearch, status, type, format, from, to, page, pageSize, setSearchParams])
  ...
  const fetchList = useCallback(async () => {
    ...
    setError(null)
    try {
      const res = await listFranchisorExports({
        search: debouncedSearch,
        status: status === 'todos' ? '' : status,
        type: type === 'todos' ? '' : type,
        format: format === 'todos' ? '' : format,
      ...
      else setError(err.message || 'Não foi possível carregar exportações. Tente novamente.')
    } finally {
      setLoading(false)
    }
  }, [permissionDenied, debouncedSearch, status, type, format, from, to, page, pageSize])
  ...
  const temFiltros = searchInput || status !== 'todos' || type !== 'todos' || format !== 'todos' || from || to

  const verDetalhes = (exportId) => {
    navigate(`/franchisor/exports/${exportId}`, { state: { returnTo: `/franchisor/exports${location.search || ''}` } })
    setMenuAberto(null)
  }
  ...
  const handleDownload = async (exportId) => {
    setMenuAberto(null)
    try {
      const { temporary_download_url } = await getFranchisorExportDownloadLink(exportId)
  ...
  return (
    <FranchisorLayout pageTitle="Exportações" breadcrumb={breadcrumb}>
      <div style={styles.cardGrande}>
        <div style={styles.cabecalho}>
          <div>
            <h2 style={styles.tituloPagina}>Exportações</h2>
            <p style={styles.subtitulo}>Gere e baixe relatórios em CSV ou PDF</p>
          </div>
          <div style={styles.cabecalhoDireita}>
            ...
            <Link to="/franchisor/exports/new" style={styles.btnPrimario} className="btn-hover">
              Nova exportação
            </Link>
          </div>
        </div>
        ...
        <div style={styles.filtros}>
          ...
          <span style={styles.filtroPeriodo}>
            <label style={styles.labelPeriodo}>Solicitada de/até</label>
            <input
              type="date"
              value={from}
        ...
        {!error && (
          <>
              ...
              <table style={styles.table}>
                <thead>
                  <tr>
                    ...
                    <th style={styles.th}>Solicitada em</th>
                  ...
                  {!loading &&
                    ...
                    data.items.map((exp) => (
                      <tr key={exp.id} style={styles.tr}>
                        ...
                        <td style={styles.td}>{getFranchisorExportTypeLabel(exp.type)}</td>
                        <td style={styles.td}>{getFranchisorExportFormatLabel(exp.format)}</td>
                        <td style={styles.td}>{exp.filters_summary || '—'}</td>
                        <td style={styles.td}>{formatFranchisorExportDate(exp.requested_at)}</td>
                        <td style={styles.td}>
                          <StatusBadge status={exp.status} />
                        </td>
                        <td style={styles.td}>{exp.expires_at ? formatFranchisorExportDate(exp.expires_at) : '—'}</td>
                        <td style={styles.td}>
            ...
            {!loading && data?.items?.length > 0 && totalPages > 0 && (
